...
public readonly record struct ReserveUnitCommand(
    bool Use10Byte,
        ...
        out ScsiCommandResult result)
    {
        if (!request.Use10Byte)
        {
            if (request.ThirdParty || request.ThirdPartyDeviceId != 0)
                throw new ArgumentOutOfRangeException(nameof(request.ThirdParty), "Third-party reservation is not supported in 6-byte RESERVE UNIT.");
